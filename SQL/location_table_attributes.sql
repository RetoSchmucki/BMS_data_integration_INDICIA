...
lav_se.int_value as "Sensitive",    -- sensitive
lav_sl.int_value as section_length_int,    -- section_length
lav_ns.int_value as nb_section_int,    -- number of section
...
lav_notelu.text_value as note_on_landuse,
lav_notehab.text_value as "Notes on habitat, land management and tenure",
lav_cn.int_value as country_id_int,    -- country ID
loc_cn.name as country_name,
loc_cn_sct.name as country_name_transect
FROM
(SELECT 
    lo.*
    FROM
    ...
    WHERE
        lo.id IN 
            ...
            WHERE 
                ...
                lo.deleted = FALSE AND
                (lo.id = 370832 OR lo.parent_id = 370832)) as loc
